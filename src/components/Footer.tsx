import React from "react";

const socialLinks = [
  {
    name: "GitHub",
    url: "https://github.com/codewithhippo17",
    color: "hover:text-ctp-mauve",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
      </svg>
    ),
  },
  {
    name: "LinkedIn",
    url: "https://linkedin.com/in/el-haiba-hamza-1628a1403",
    color: "hover:text-ctp-blue",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    name: "X (Twitter)",
    url: "https://x.com/elhaiba__hamza",
    color: "hover:text-ctp-lavender",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    name: "Reddit",
    url: "https://www.reddit.com/u/LongFaithlessness366/s/ai1Wg19f6I",
    color: "hover:text-ctp-red",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.825.07 3.48.632 4.674 1.488a1.745 1.745 0 0 1 1.065-.357c.965 0 1.75.785 1.75 1.75 0 .717-.44 1.327-1.055 1.591.025.175.04.351.04.53 0 2.953-3.284 5.348-7.325 5.348S5.02 16.703 5.02 13.75c0-.179.015-.355.04-.53a1.773 1.773 0 0 1-1.005-1.591c0-.965.785-1.75 1.75-1.75.398 0 .768.14 1.065.357 1.194-.856 2.85-1.418 4.675-1.488l.799-3.747a.3.3 0 0 1 .347-.245l3.248.684a1.247 1.247 0 0 1 1.086-.646zM8.25 11.81c-.719 0-1.25.542-1.25 1.25 0 .709.531 1.251 1.25 1.251.718 0 1.25-.542 1.25-1.25 0-.709-.532-1.251-1.25-1.251zm3.75.001a.75.75 0 0 0-.75.75.75.75 0 0 0 .75.75.75.75 0 0 0 .75-.75.75.75 0 0 0-.75-.75zm-2.032 2.668a.685.685 0 0 0-.397.192.535.535 0 0 0-.007.743.533.533 0 0 0 .743.007 2.88 2.88 0 0 1 1.943-.694c.744 0 1.416.266 1.943.694a.533.533 0 0 0 .743-.007.535.535 0 0 0-.007-.743.685.685 0 0 0-.397-.192 3.94 3.94 0 0 0-2.282 0z" />
      </svg>
    ),
  },
  {
    name: "Medium",
    url: "https://medium.com/@elhaiba.hamza",
    color: "hover:text-ctp-green",
    icon: (
      <svg role="img" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M4.21 0A4.201 4.201 0 0 0 0 4.21v15.58A4.201 4.201 0 0 0 4.21 24h15.58A4.201 4.201 0 0 0 24 19.79v-1.093c-.137.013-.278.02-.422.02-2.577 0-4.027-2.146-4.09-4.832a7.592 7.592 0 0 1 .022-.708c.093-1.186.475-2.241 1.105-3.022a3.885 3.885 0 0 1 1.395-1.1c.468-.237 1.127-.367 1.664-.367h.023c.101 0 .202.004.303.01V4.211A4.201 4.201 0 0 0 19.79 0Zm.198 5.583h4.165l3.588 8.435 3.59-8.435h3.864v.146l-.019.004c-.705.16-1.063.397-1.063 1.254h-.003l.003 10.274c.06.676.424.885 1.063 1.03l.02.004v.145h-4.923v-.145l.019-.005c.639-.144.994-.353 1.054-1.03V7.267l-4.745 11.15h-.261L6.15 7.569v9.445c0 .857.358 1.094 1.063 1.253l.02.004v.147H4.405v-.147l.019-.004c.705-.16 1.065-.397 1.065-1.253V6.987c0-.857-.358-1.094-1.064-1.254l-.018-.004zm19.25 3.668c-1.086.023-1.733 1.323-1.813 3.124H24V9.298a1.378 1.378 0 0 0-.342-.047Zm-1.862 3.632c-.1 1.756.86 3.239 2.204 3.634v-3.634z" />
      </svg>
    ),
  },
  {
    name: "Email",
    url: "mailto:elhaiba.hamza@proton.me",
    color: "hover:text-ctp-peach",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <rect x="2" y="4" width="20" height="16" rx="2" />
        <path d="M22 4l-10 8L2 4" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="pt-16 pb-6 flex flex-col justify-between">
      <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-12">

        {/* Left Side: Branding / Identity */}
        <div className="space-y-6 max-w-sm">
          <div>
            <h2 className="text-3xl font-extrabold text-ctp-text tracking-tight mb-2">Hamza El Haiba</h2>
            <p className="text-ctp-mauve font-medium tracking-wide text-sm uppercase">Architect in Digital Technologies</p>
          </div>
          <p className="text-ctp-subtext0 leading-relaxed text-sm">
            I don't draw perfect, static blueprints. I draw strict boundaries and relax them only when necessary. Building systems that gracefully outgrow their assumptions.
          </p>
        </div>

        {/* Right Side: Contact Directory */}
        <div className="flex flex-col md:items-end w-full md:w-auto">
          <span className="text-xs font-bold uppercase tracking-widest text-ctp-surface2 mb-6">Elsewhere</span>
          <div className="grid grid-cols-2 gap-x-12 gap-y-4">
            {socialLinks.map((link) => (
              <a
                key={link.name}
                href={link.url}
                target={link.name !== "Email" ? "_blank" : "_self"}
                rel={link.name !== "Email" ? "noopener noreferrer" : ""}
                className={`flex items-center gap-3 text-ctp-subtext0 transition-all duration-300 group ${link.color}`}
                aria-label={link.name}
              >
                <div className="text-ctp-surface2 group-hover:text-inherit transition-colors duration-300">
                  {link.icon}
                </div>
                <span className="text-sm font-medium">{link.name}</span>
              </a>
            ))}
          </div>
        </div>
      </div>

      {/* Massive Typography Watermark */}
      <div className="w-full overflow-hidden mb-8 flex justify-center opacity-30 select-none pointer-events-none">
        <span className="text-[10vw] font-black text-ctp-surface0 leading-[0.75] tracking-tighter">
          RESILIENT.
        </span>
      </div>

      {/* Bottom Legal Row */}
      <div className="flex flex-col md:flex-row items-center justify-between pt-6 border-t border-ctp-surface0/50 text-xs text-ctp-overlay0 gap-4">
        <p>© {new Date().getFullYear()} Hamza El Haiba. All rights reserved.</p>
        <p className="flex items-center gap-1">
          Based in Morocco <span className="text-ctp-surface2">•</span> GMT+1
        </p>
      </div>
    </footer>
  );
}
